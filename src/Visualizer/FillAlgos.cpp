...
static bool isSquareBoundary(int x, int y, int cx, int cy, int h) {
  ...
  if (x < lo_x || x > hi_x || y < lo_y || y > hi_y)
    return false; // out of box entirely
...
FloodFill::FloodFill(bool eightWay)
    : is8Way(eightWay), seedX(10), seedY(10), halfS(8), currentStep(0),
      totalSteps(0) {
  buildObstacles();
}
...
bool FloodFill::isValid(int x, int y) const {
  if (!isInsideBox(x, y, seedX, seedY, halfS))
    return false;
  if (filledRaw.count({x, y}))
    return false;
  return true;
}
...
void FloodFill::step() {
  ...
  lastState.calcLines.push_back("--- Step " + ei(currentStep) + " / " +
                                ei(totalSteps) + " ---");
  lastState.calcLines.push_back("  Popped: (" + ei(p.x) + ", " + ei(p.y) +
                                ") -> FILLED");
  lastState.calcLines.push_back(
      "  Boundary: x in [" + ei(seedX - halfS) + ".." + ei(seedX + halfS) +
      "]  y in [" + ei(seedY - halfS) + ".." + ei(seedY + halfS) + "]");
  lastState.calcLines.push_back("  Enqueued " + ei(enqueued) +
                                " new neighbour(s).");
  lastState.calcLines.push_back("  Frontier Queue Size: " +
                                ei((int)frontier.size()));
  lastState.calcLines.push_back("");
  if (isFinished())
    lastState.calcLines.push_back("Queue empty — Flood Fill complete!");
  else
    lastState.calcLines.push_back("Next pixel: (" + ei(frontier.front().x) +
                                  ", " + ei(frontier.front().y) + ")");
}
...
void FloodFill::stepK(int k) {
  ...
  lastState.calcLines.clear();
  lastState.calcLines.push_back(
      "Stepped " + ei(k) + " silently.  Queue: " + ei((int)frontier.size()));
}
...
std::vector<std::string> FloodFill::getCurrentVars() const {
  return {"Queue Size : " + ei((int)frontier.size()),
          "Filled     : " + ei(currentStep), "Total Goal : " + ei(totalSteps),
          "Half-side  : " + ei(halfS)};
}
...
void BoundaryFill::step() {
  ...
  lastState.calcLines.push_back("  Boundary : " + ei(boundaryHits) +
                                "  (boundary color -> blocked)");
  ...
  if (isFinished())
    ...
    lastState.calcLines.push_back("  Next: (" + ei(frontier.front().x) + "," +
                                  ei(frontier.front().y) + ")");
}
...
void BoundaryFill::runToCompletion() {
  while (!isFinished())
    stepK(100);
}
...
ScanlineFill::ScanlineFill()
    : cx(10), cy(10), polySize(7), numSides(5), currentY(0), yMinPoly(0),
      yMaxPoly(0), currentStep(0), totalSteps(0), done(false) {}
...
void ScanlineFill::generatePolygon() {
  ...
  for (int i = 0; i < numSides; i++) {
    ...
    vertices.push_back(std::make_pair((float)cx + (float)polySize * cosf(a),
...
void ScanlineFill::doOneStep() {
  ...
  lastState.calcLines.push_back("--- Scanline Y = " + ei(currentY) +
                                "  (step " + ei(currentStep + 1) + "/" +
                                ei(totalSteps) + ") ---");
  lastState.calcLines.push_back("");
  lastState.calcLines.push_back("1) Added from ET[" + ei(currentY) +
                                "]:  " + ei(newCount) + " edge(s)");
  lastState.calcLines.push_back(
      "2) AET size (after removal+sort): " + ei((int)activeEdges.size()) +
      " active edge(s)");
  lastState.calcLines.push_back("");
  ...
  if (spans.empty()) {
    ...
    for (auto &sp : spans)
      lastState.calcLines.push_back("   x = [" + ei(sp.first) + " .. " +
                                    ei(sp.second) + "]  -> " +
  ...
  lastState.calcLines.push_back("");
  lastState.calcLines.push_back(
      "4) Update x += (1/slope) for Y=" + ei(currentY + 1) + ":");
  for (int i = 0; i < (int)activeEdges.size(); i++) {
    std::ostringstream os;
    os << std::fixed << std::setprecision(2);
    os << "   [" << i << "] " << xBefore[i] << " + " << activeEdges[i].invSlope
       << " = " << activeEdges[i].x;
    lastState.calcLines.push_back(os.str());
  }
...
void ScanlineFill::stepK(int k) {
  ...
  lastState.hasCalculation = false;
  lastState.calcLines.clear();
  lastState.calcLines.push_back("Stepped " + ei(k) +
                                " scanlines.  Y now: " + ei(currentY));
}
