...
DDALine::DDALine()
    : x1(0), y1(0), x2(0), y2(0), currentX(0), currentY(0), xIncrement(0),
      yIncrement(0), totalSteps(0), currentStep(0) {}

void DDALine::calculateIncrements() {
  int dx = x2 - x1, dy = y2 - y1;
  totalSteps = std::max(std::abs(dx), std::abs(dy));
  if (totalSteps > 0) {
    xIncrement = (float)dx / totalSteps;
    yIncrement = (float)dy / totalSteps;
  } else {
...
void DDALine::step() {
  ...
  lastState.calcLines.push_back("  x  =  " + f4(px) + " + " + f4(xIncrement) +
                                " = " + f4(currentX));
  lastState.calcLines.push_back("--- Y update ---");
  lastState.calcLines.push_back("  y  =  y_prev + yInc");
  lastState.calcLines.push_back("  y  =  " + f4(py) + " + " + f4(yIncrement) +
                                " = " + f4(currentY));
...
void DDALine::stepK(int k) {
  for (int i = 0; i < k && !isFinished(); ++i) {
    ...
    currentStep++;
    path.push_back(
        {(int)std::round(currentX), (int)std::round(currentY), 1.0f});
  }
...
std::vector<std::string> DDALine::getCurrentVars() const {
  return {"Float X  : " + f4(currentX),
          "Float Y  : " + f4(currentY),
          "xInc     : " + f4(xIncrement),
          "yInc     : " + f4(yIncrement),
          "Pixel    : (" + si((int)std::round(currentX)) + ", " +
              si((int)std::round(currentY)) + ")",
...
BresenhamLine::BresenhamLine()
    : x1(0), y1(0), x2(0), y2(0), x(0), y(0), dx(0), dy(0), sx(1), sy(1), p(0),
      steep(false), totalSteps(0), currentStep(0) {}
...
void BresenhamLine::step() {
  if (isFinished())
    return;
  int op = p, ox_ = x, oy_ = y;
  bool ym = false, xm = false;
  if (!steep) {
    x += sx;
    if (p < 0) {
      p += 2 * dy;
  ...
  if (!steep) {
    ...
    lastState.calcLines.push_back("  x = " + si(ox_) + " + " + si(sx) + " = " +
                                  si(x));
    ...
    if (!ym) {
      ...
      lastState.calcLines.push_back("  p = p + 2*dy = " + si(op) + " + " +
                                    si(2 * dy) + " = " + si(p));
    } else {
      lastState.calcLines.push_back("  p >= 0 ?  YES  (" + si(op) + " >= 0)");
      lastState.calcLines.push_back("  Decision: Y increments");
      lastState.calcLines.push_back("  y = " + si(oy_) + " + " + si(sy) +
                                    " = " + si(y));
      lastState.calcLines.push_back("  p = p + 2*dy - 2*dx = " + si(op) +
                                    " + " + si(2 * dy) + " - " + si(2 * dx) +
                                    " = " + si(p));
    }
  } else {
    ...
    if (!xm) {
      ...
      lastState.calcLines.push_back("  p = p + 2*dx = " + si(op) + " + " +
                                    si(2 * dx) + " = " + si(p));
    } else {
      lastState.calcLines.push_back("  p >= 0 ?  YES  (" + si(op) + " >= 0)");
      ...
      lastState.calcLines.push_back("  x = " + si(ox_) + " + " + si(sx) +
                                    " = " + si(x));
      lastState.calcLines.push_back("  p = p + 2*dx - 2*dy = " + si(op) +
                                    " + " + si(2 * dx) + " - " + si(2 * dy) +
                                    " = " + si(p));
    }
  }
  lastState.currentPixelInfo =
      ">> Pixel plotted: (" + si(x) + ", " + si(y) + ")";
}
...
std::string BresenhamLine::getName() const {
  return "Bresenham's Line Algorithm";
}

std::vector<std::string> BresenhamLine::getInitInfo() const {
  std::string mode = steep ? "Steep (|dy|>|dx|) — primary: Y"
                           : "Gentle (|dx|>=|dy|) — primary: X";
  int p0 = steep ? 2 * dx - dy : 2 * dy - dx;
  std::string pf =
      steep ? "p0 = 2*dx-dy = 2*" + si(dx) + "-" + si(dy) + "=" + si(p0)
            : "p0 = 2*dy-dx = 2*" + si(dy) + "-" + si(dx) + "=" + si(p0);
  return {"dx=|" + si(x2) + "-" + si(x1) + "|=" + si(dx) + "  dy=|" + si(y2) +
              "-" + si(y1) + "|=" + si(dy),
          "sx=" + si(sx) + " (X direction)  sy=" + si(sy) + " (Y direction)",
          "Slope mode: " + mode, pf, "Total steps = " + si(totalSteps)};
}
std::vector<std::string> BresenhamLine::getCurrentVars() const {
  return {"Current X  : " + si(x),
          "Current Y  : " + si(y),
          "Decision p : " + si(p),
          "dx         : " + si(dx),
          "dy         : " + si(dy),
...
XiaolinWuLine::XiaolinWuLine()
    : ox0(0), oy0(0), ox1(0), oy1(0), fpx0(0), fpy0(0), fpx1(0), fpy1(0),
      steep(false), gradient(0), xpxl1(0), xpxl2(0), yend1(0), xgap1(0),
      yend2(0), xgap2(0), intery(0), currentX(0), currentStep(0),
      totalSteps(0) {}

void XiaolinWuLine::plotPixel(int x, int y, float intensity) {
  intensity = std::max(0.0f, std::min(1.0f, intensity));
  if (steep)
    path.push_back({y, x, intensity});
  else
    path.push_back({x, y, intensity});
}
...
void XiaolinWuLine::step() {
  ...
  if (preX == xpxl1) {
    ...
    lastState.calcLines.push_back("  X column = xpxl1 = round(" + f4(fpx0) +
                                  ") = " + si(xpxl1));
    lastState.calcLines.push_back("  yend  = y0 + grad*(xpxl1-x0)");
    lastState.calcLines.push_back("       = " + f4(fpy0) + " + " +
                                  f4(gradient) + "*(" + si(xpxl1) + "-" +
                                  f4(fpx0) + ")");
    lastState.calcLines.push_back("       = " + f4(yend1));
    lastState.calcLines.push_back("  xgap = rfrac(x0+0.5) = rfrac(" +
                                  f4(fpx0 + 0.5f) + ") = " + f4(xgap1));
    lastState.calcLines.push_back("");
    lastState.calcLines.push_back("--- Intensity Calculation ---");
    lastState.calcLines.push_back("  ipart(yend) = " + si(iy) +
                                  "  (lower row)");
    lastState.calcLines.push_back("  rfrac(yend) = " + f4(wu_rfrac(yend1)) +
                                  "  (how close to lower row)");
    lastState.calcLines.push_back(
        "  i1 = rfrac(yend)*xgap = " + f4(wu_rfrac(yend1)) + "*" + f4(xgap1) +
        " = " + f4(i1) + " (" + pct(i1) + ")");
    lastState.calcLines.push_back("  frac(yend) = " + f4(wu_frac(yend1)));
    lastState.calcLines.push_back(
        "  i2 = frac(yend)*xgap  = " + f4(wu_frac(yend1)) + "*" + f4(xgap1) +
        " = " + f4(i2) + " (" + pct(i2) + ")");
    ...
    lastState.currentPixelInfo = ">> (" + si(px1) + "," + si(py1) +
                                 ") i=" + f4(i1) + "   (" + si(px2) + "," +
                                 si(py2) + ") i=" + f4(i2);
    ...
    lastState.calcLines.push_back("  X column = xpxl2 = round(" + f4(fpx1) +
                                  ") = " + si(xpxl2));
    lastState.calcLines.push_back("  yend  = y1 + grad*(xpxl2-x1)");
    lastState.calcLines.push_back("       = " + f4(fpy1) + " + " +
                                  f4(gradient) + "*(" + si(xpxl2) + "-" +
                                  f4(fpx1) + ")");
    lastState.calcLines.push_back("       = " + f4(yend2));
    lastState.calcLines.push_back("  xgap = frac(x1+0.5) = frac(" +
                                  f4(fpx1 + 0.5f) + ") = " + f4(xgap2));
    lastState.calcLines.push_back("");
    lastState.calcLines.push_back("--- Intensity Calculation ---");
    lastState.calcLines.push_back("  ipart(yend) = " + si(iy));
    lastState.calcLines.push_back(
        "  i1 = rfrac(yend)*xgap = " + f4(wu_rfrac(yend2)) + "*" + f4(xgap2) +
        " = " + f4(i1) + " (" + pct(i1) + ")");
    lastState.calcLines.push_back(
        "  i2 = frac(yend)*xgap  = " + f4(wu_frac(yend2)) + "*" + f4(xgap2) +
        " = " + f4(i2) + " (" + pct(i2) + ")");
    ...
    lastState.calcLines.push_back("  intery    = " + f4(preIntery) +
                                  "  (floating Y position)");
    lastState.calcLines.push_back(
        "  next intery += gradient: " + f4(preIntery) + "+" + f4(gradient) +
        "=" + f4(preIntery + gradient));
    lastState.calcLines.push_back("");
    lastState.calcLines.push_back("--- Intensity Calculation ---");
    lastState.calcLines.push_back("  ipart(intery) = floor(" + f4(preIntery) +
                                  ") = " + si(iy));
    lastState.calcLines.push_back("  frac(intery)  = " + f4(preIntery) + "-" +
                                  si(iy) + " = " + f4(f));
    lastState.calcLines.push_back("  rfrac(intery) = 1 - " + f4(f) + " = " +
                                  f4(rf));
    lastState.calcLines.push_back("");
    lastState.calcLines.push_back(
        "  frac  tells us: how far ABOVE the lower row the line is");
    lastState.calcLines.push_back(
        "  rfrac tells us: how close to the LOWER row");
    lastState.calcLines.push_back("");

    // Colour intensity bars (ASCII visualization)
    int barLow = (int)(rf * 20.0f);
    int barHigh = (int)(f * 20.0f);
    std::string barL(barLow, '#');
    barL += std::string(20 - barLow, '.');
    std::string barH(barHigh, '#');
    barH += std::string(20 - barHigh, '.');
    ...
    lastState.calcLines.push_back("  Lower pixel (" + si(px1) + "," + si(py1) +
                                  "): rfrac = " + f4(rf) + " (" + pct(rf) +
                                  ") [" + barL + "]");
    lastState.calcLines.push_back("  Upper pixel (" + si(px2) + "," + si(py2) +
                                  "): frac  = " + f4(f) + " (" + pct(f) +
                                  ") [" + barH + "]");
    ...
    lastState.currentPixelInfo = ">> Lower:(" + si(px1) + "," + si(py1) +
                                 ") i=" + f4(rf) + "  Upper:(" + si(px2) + "," +
                                 si(py2) + ") i=" + f4(f);
  }
...
std::string XiaolinWuLine::getName() const {
  return "Xiaolin Wu's Anti-Aliased Line";
}
...
std::string XiaolinWuLine::getTheory() const {
  return "Xiaolin Wu's Anti-Aliased Line Algorithm\n"
         "=========================================\n\n"
         "WHAT IS IT?\n"
