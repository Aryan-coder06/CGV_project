...
MidpointCircle::MidpointCircle()
    : cx(0), cy(0), r(8), x_(0), y_(0), p_(0), currentStep(0), totalSteps(0) {}
...
void MidpointCircle::step() {
  ...
  lastState.calcLines.push_back("  x = " + ci(old_x) + "  y = " + ci(old_y) +
                                "  p = " + ci(old_p));
  ...
  if (!y_decremented) {
    ...
    lastState.calcLines.push_back("  Decision: UPPER pixel (Y stays at " +
                                  ci(y_) + ")");
  ...
  lastState.calcLines.push_back("--- 8-Fold Symmetry  (x=" + ci(x_) +
                                ", y=" + ci(y_) + ") ---");
  lastState.calcLines.push_back("  (+x,+y)=(" + ci(cx + x_) + "," +
                                ci(cy + y_) + ")  " + "(-x,+y)=(" +
                                ci(cx - x_) + "," + ci(cy + y_) + ")");
  lastState.calcLines.push_back("  (+x,-y)=(" + ci(cx + x_) + "," +
                                ci(cy - y_) + ")  " + "(-x,-y)=(" +
                                ci(cx - x_) + "," + ci(cy - y_) + ")");
  lastState.calcLines.push_back("  (+y,+x)=(" + ci(cx + y_) + "," +
                                ci(cy + x_) + ")  " + "(-y,+x)=(" +
                                ci(cx - y_) + "," + ci(cy + x_) + ")");
  lastState.calcLines.push_back("  (+y,-x)=(" + ci(cx + y_) + "," +
                                ci(cy - x_) + ")  " + "(-y,-x)=(" +
  ...
  lastState.currentPixelInfo =
      ">> New position: (" + ci(x_) + "," + ci(y_) + ")  p=" + ci(p_);
}
...
std::string MidpointCircle::getName() const {
...
std::vector<std::string> MidpointCircle::getCurrentVars() const {
  ...
  float midpointF = (float)(x_ + 1) * (x_ + 1) +
                    ((float)y_ - 0.5f) * ((float)y_ - 0.5f) - (float)r * r;
  return {
      "Current x  : " + ci(x_),
      "Current y  : " + ci(y_),
      "Decision p : " + ci(p_),
      "r          : " + ci(r),
      "x < y ?    : " +
          std::string(x_ < y_ ? "YES (still looping)" : "NO  (done)"),
      "F(x+1,y-0.5)= " + cf4(midpointF) +
          (midpointF < 0 ? " < 0 (inside)" : " >= 0 (outside)"),
...
BresenhamCircle::BresenhamCircle()
    : cx(0), cy(0), r(8), x_(0), y_(0), d_(0), currentStep(0), totalSteps(0) {}
...
void BresenhamCircle::step() {
  ...
  lastState.calcLines.push_back("--- Plotting at (x=" + ci(preX) +
                                ", y=" + ci(preY) + ") ---");
  lastState.calcLines.push_back("  d (current) = " + ci(preD));
  lastState.calcLines.push_back("");
  lastState.calcLines.push_back("--- 8-Fold Symmetry Plotted ---");
  lastState.calcLines.push_back("  (+x,+y)=(" + ci(cx + preX) + "," +
                                ci(cy + preY) + ")  " + "(-x,+y)=(" +
                                ci(cx - preX) + "," + ci(cy + preY) + ")");
  lastState.calcLines.push_back("  (+x,-y)=(" + ci(cx + preX) + "," +
                                ci(cy - preY) + ")  " + "(-x,-y)=(" +
                                ci(cx - preX) + "," + ci(cy - preY) + ")");
  lastState.calcLines.push_back("  (+y,+x)=(" + ci(cx + preY) + "," +
                                ci(cy + preX) + ")  " + "(-y,+x)=(" +
                                ci(cx - preY) + "," + ci(cy + preX) + ")");
  lastState.calcLines.push_back("  (+y,-x)=(" + ci(cx + preY) + "," +
                                ci(cy - preX) + ")  " + "(-y,-x)=(" +
                                ci(cx - preY) + "," + ci(cy - preX) + ")");
  lastState.calcLines.push_back("");
  ...
  if (!y_dec) {
    ...
    lastState.calcLines.push_back("  d_new = " + ci(preD) + " + 4*(" +
                                  ci(preX) + " - " + ci(preY) +
                                  ") + 10 = " + ci(d_));
  }
  lastState.calcLines.push_back("");
  ...
  lastState.calcLines.push_back("  Next state: x=" + ci(x_) + "  y=" + ci(y_) +
                                "  d=" + ci(d_));

  lastState.currentPixelInfo = ">> Plotted (" + ci(preX) + "," + ci(preY) +
                               ")  next: x=" + ci(x_) + ", y=" + ci(y_) +
                               ", d=" + ci(d_);
}
...
std::vector<Pixel> BresenhamCircle::getHighlightedPixels() const {
  return path;
}
AlgoState BresenhamCircle::getCurrentState() const { return lastState; }
std::string BresenhamCircle::getName() const {
  return "Bresenham's Circle Algorithm";
}
