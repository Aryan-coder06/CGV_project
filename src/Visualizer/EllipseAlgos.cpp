...
MidpointEllipse::MidpointEllipse()
    : cx(0), cy(0), a(10), b(6), a2_(0), b2_(0), x_(0), y_(0), p1_(0), p2_(0),
...
void MidpointEllipse::computeTotalSteps() {
  ...
  float tp2 = fb2 * ((float)tx + 0.5f) * ((float)tx + 0.5f) +
              fa2 * (float)(ty - 1) * (float)(ty - 1) - fa2 * fb2;
...
void MidpointEllipse::stepRegion1() {
  ...
  lastState.calcLines.push_back("  (+x,+y) = (" + ei(cx + preX) + "," +
                                ei(cy + preY) + ")  " + "(-x,+y) = (" +
                                ei(cx - preX) + "," + ei(cy + preY) + ")");
  if (preY != 0)
    lastState.calcLines.push_back("  (+x,-y) = (" + ei(cx + preX) + "," +
                                  ei(cy - preY) + ")  " + "(-x,-y) = (" +
                                  ei(cx - preX) + "," + ei(cy - preY) + ")");
  ...
  lastState.calcLines.push_back("  Midpoint at (x+1, y-0.5) = (" +
                                ei(preX + 1) + ", " + ef2(preY - 0.5f) + ")");
  if (!y_dec) {
    lastState.calcLines.push_back("  p1 < 0 ?  YES  (" + ef2(preP) + " < 0)");
    lastState.calcLines.push_back("  Decision: Midpoint INSIDE — Y stays at " +
                                  ei(preY));
    lastState.calcLines.push_back("  x_new = " + ei(preX) + " + 1 = " + ei(x_));
    lastState.calcLines.push_back("  p1_new = p1 + 2*b²*x_new + b²");
    lastState.calcLines.push_back("  p1_new = " + ef2(preP) + " + 2*" +
                                  ei(b2_) + "*" + ei(x_) + " + " + ei(b2_) +
                                  " = " + ef2(p1_));
  } else {
    lastState.calcLines.push_back("  p1 >= 0 ?  YES  (" + ef2(preP) + " >= 0)");
    lastState.calcLines.push_back(
        "  Decision: Midpoint OUTSIDE — Y decrements");
    lastState.calcLines.push_back("  x_new = " + ei(preX) + " + 1 = " + ei(x_));
    lastState.calcLines.push_back("  y_new = " + ei(preY) + " - 1 = " + ei(y_));
    lastState.calcLines.push_back(
        "  p1_new = p1 + 2*b²*x_new + b² - 2*a²*y_new");
    lastState.calcLines.push_back(
        "  p1_new = " + ef2(preP) + " + 2*" + ei(b2_) + "*" + ei(x_) + " + " +
        ei(b2_) + " - 2*" + ei(a2_) + "*" + ei(y_) + " = " + ef2(p1_));
  }
  lastState.calcLines.push_back("");
...
void MidpointEllipse::stepRegion2() {
  ...
  if (!x_inc) {
    ...
    y_--;
    x_++;
    p2_ += 2.0f * (float)b2_ * (float)x_ - 2.0f * (float)a2_ * (float)y_ +
           (float)a2_;
  }
  currentStep++;
  if (y_ < 0)
  ...
  lastState.calcLines.push_back("  Midpoint at (x+0.5, y-1) = (" +
                                ef2(preX + 0.5f) + ", " + ei(preY - 1) + ")");
  if (!x_inc) {
    lastState.calcLines.push_back("  p2 > 0 ?  YES  (" + ef2(preP) + " > 0)");
    lastState.calcLines.push_back("  Decision: Midpoint OUTSIDE — X stays at " +
                                  ei(preX));
    lastState.calcLines.push_back("  y_new = " + ei(preY) + " - 1 = " + ei(y_));
    lastState.calcLines.push_back("  p2_new = p2 + a² - 2*a²*y_new");
    lastState.calcLines.push_back("  p2_new = " + ef2(preP) + " + " + ei(a2_) +
                                  " - 2*" + ei(a2_) + "*" + ei(y_) + " = " +
                                  ef2(p2_));
  } else {
    lastState.calcLines.push_back("  p2 <= 0 ?  YES  (" + ef2(preP) + " <= 0)");
    ...
    lastState.calcLines.push_back("  x_new = " + ei(preX) + " + 1 = " + ei(x_));
    lastState.calcLines.push_back(
        "  p2_new = p2 + 2*b²*x_new - 2*a²*y_new + a²");
    lastState.calcLines.push_back(
        "  p2_new = " + ef2(preP) + " + 2*" + ei(b2_) + "*" + ei(x_) + " - 2*" +
        ei(a2_) + "*" + ei(y_) + " + " + ei(a2_) + " = " + ef2(p2_));
  }
...
static void ellipseSilentStep1(int &x_, int &y_, float &p1_, float a2f,
                               float b2f, int a2, int b2) {
  if (p1_ < 0.0f) {
    x_++;
    p1_ += 2.0f * b2f * x_ + b2f;
  } else {
  ...
  (void)a2;
  (void)b2; // suppress warnings
}

static void ellipseSilentStep2(int &x_, int &y_, float &p2_, float a2f,
                               float b2f) {
  if (p2_ > 0.0f) {
    y_--;
...
void MidpointEllipse::stepK(int k) {
  ...
  for (int i = 0; i < k && !isFinished(); ++i) {
    ...
    if (region_ == EllipseRegion::REGION1) {
      ...
      currentStep++;
      if (2 * b2_ * x_ >= 2 * a2_ * y_) {
        p2_ = b2f * (x_ + 0.5f) * (x_ + 0.5f) +
              a2f * (float)(y_ - 1) * (float)(y_ - 1) - a2f * b2f;
        region_ = EllipseRegion::REGION2;
      }
    } else if (region_ == EllipseRegion::REGION2) {
      plotSymmetric(x_, y_, dummy);
...
bool MidpointEllipse::isFinished() const {
  return region_ == EllipseRegion::DONE || currentStep >= totalSteps;
}
std::vector<Pixel> MidpointEllipse::getHighlightedPixels() const {
  return path;
}
AlgoState MidpointEllipse::getCurrentState() const { return lastState; }
std::string MidpointEllipse::getName() const {
  return "Midpoint Ellipse Algorithm";
}

std::vector<std::string> MidpointEllipse::getInitInfo() const {
  float p1init = (float)b2_ - (float)a2_ * b + 0.25f * (float)a2_;
  std::string reg1cond =
      "2*b²*x < 2*a²*y  i.e.  2*" + ei(b2_) + "*x < 2*" + ei(a2_) + "*y";
  return {
      "Centre:   (" + ei(cx) + ", " + ei(cy) + ")",
...
std::vector<std::string> MidpointEllipse::getCurrentVars() const {
  std::string regStr = (region_ == EllipseRegion::REGION1) ? "1  (x increments)"
                       : (region_ == EllipseRegion::REGION2)
                           ? "2  (y decrements)"
                           : "Done";
  ...
  v.push_back("a=" + ei(a) + "  b=" + ei(b) + "  a²=" + ei(a2_) +
              "  b²=" + ei(b2_));
  if (region_ == EllipseRegion::REGION1) {
